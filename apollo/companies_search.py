...
class ApolloAPI:
    ...
    ORGANIZATION_ENRICH_ENDPOINT = (
        f"{BASE_API_ENDPOINT}/{APOLLO_BASE_API_VERSION}/organizations/enrich"
    )
    ORGANIZATION_BULK_ENRICH_ENDPOINT = (
        f"{BASE_API_ENDPOINT}/{APOLLO_BASE_API_VERSION}/organizations/bulk_enrich"
    )
    ...
    def enrich_lead(self, domain: str) -> ApolloResult:
        ...
        try:
            r = requests.post(
                self.ORGANIZATION_ENRICH_ENDPOINT, headers=self.HEADERS, json={"domain": domain}
            )
