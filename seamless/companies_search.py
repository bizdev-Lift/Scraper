...
class SeamlessAPI:
    ...
    COMPANIES_ENRICH_ENDPOINT = (
        f"{BASE_API_ENDPOINT}/{SEAMLESS_BASE_API_VERSION}/companies/research"
    )
    COMPANIES_POLL_ENDPOINT = (
        f"{BASE_API_ENDPOINT}/{SEAMLESS_BASE_API_VERSION}/companies/research/poll"
    )
    HEADERS = {"Content-Type": "application/json", "Token": settings.seamless_api_key}
                    ...
                    for item, requested_domain in zip(results, domains):
                        ...
                        if status == "done":
                            ...
                            if not self._domains_match(requested_domain, returned_domain):
                                logger.info(
                                    f"[Seamless] Domain mismatch — asked for '{requested_domain}', "
                            ...
                            logger.info(
                                f"[Seamless] Still researching... (poll {attempt}/{self.SEAMLESS_MAX_POLLS})"
                            )
                            time.sleep(self.SEAMLESS_POLL_WAIT)
                            retry = True
                            break
                            ...
                            logger.error(
                                f"[Seamless] Research status: {status} (check credits/license if 'error') for {requested_domain}"
                            )
    ...
    def _empty_seamless(self, domains: list[str]) -> dict:
        return {domain: SeamlessResult() for domain in domains}
